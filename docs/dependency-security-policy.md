...
- JavaScript: `cloudflare/workers-sdk`, `honojs/hono`, `honojs/middleware`,
  ...
  `microsoft/TypeScript`, `microsoft/playwright`, `vitest-dev/vitest`,
  `colinhacks/zod`, `vercel/turborepo`, `lovell/sharp`, and `oven-sh/bun`.
...
audited independently in CI in addition to the root Bun lockfile, so the
container cannot silently resolve a different browser harness dependency
graph.
